...
class Viewopenstat extends CI_Controller {
	...
	public function index($id='',$page='',$open='',$img=''){		
		$data["msturi"]   = "http://localhost:88/PRIORITAS/FLIPBOOK/88/";
		if(!empty($img)){
			...
			if(empty($id)){
			    ...
			    $data["halaman"]  = $page;
			    if(!empty($open)){
	   				#$this->load->view("backend/202101_1/openview_template_single",$data);
			    }else{
			    	if(!empty($page)){
					    $this->load->view("templates/202101_1/openview_header",$data);
					    $this->load->view("backend/202101_1/openview_template_stat",$data);
					    $this->load->view("templates/202101_1/openview_footer");
					}else{
						$this->load->view("backend/202101_1/openview_template_stat",$data);						
					}
				}
