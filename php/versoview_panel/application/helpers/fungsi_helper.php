...
if (!function_exists('popup')) {
	function popup($no, $sintak)
	{
		...
		echo "<pre style='background:tomato;position:fixed;color:#fff;font-size:14px;z-index:9999999;padding:5px 20px;;$no'>" . print_r($sintak, true) . "</pre>";
	}
}
...
if (!function_exists('ov_img_dtl')) {
	function ov_img_dtl($path, $tipe)
	{
		if ($_SERVER["SERVER_NAME"] == "versoview.com" || $_SERVER["SERVER_NAME"] == "panel.versoview.com") {
			...
			if (strpos($path, 'http') !== false) {
				...
				$url = 'http://localhost/versoview/pageturner/' . $path;
			}
		}
...
if (!function_exists('get_img_user')) {
	...
	{
		#h3($img_user.$prov);
		$img_path = "gallery/user/" . $img_user;
...
if (!function_exists('cover')) {
	function cover($pdfname, $cvrname)
	{
		if (DIRECTORY_SEPARATOR == '\\') {
			shell_exec(sys('conv') . ' -colorspace sRGB -density  40 ' . $pdfname . '[0] -set units PixelsPerInch -alpha remove  -quality 50 ' . $cvrname);
		} else {
			shell_exec("pdftocairo -jpeg -singlefile  -f 1 -l 1 " . $pdfname . " " . str_replace(".jpg", "", $cvrname));
		}
	}
}
