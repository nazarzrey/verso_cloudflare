...
class Backend extends CI_Controller {
	function __construct() {
    	...
    	$this->load->model(array('Mod_login','Mod_user','User_activity','Mod_query'));
	}
	#index
	public function index($value=""){
		if($this->session->userdata()){		
			$data["data"] = $this->Mod_user->userownmagazine($this->session->userdata('uid'));
			$form   = array();
		...
		#h3($value.$value1.$value2);
      	if(!empty($value)){
      		if(empty($value1)){
		    	...
		    	if($value1=="setting"){
					h3("global setting magazine");
		      	}else{
			  		$data['datamagazine'] = $this->Mod_query->getDataMagazine($value);
			      	$data['issuemagazine'] = $this->Mod_query->issueMagazine($this->Mod_query->getDataMagazine($value)->magz_fk_issue);
			      	$data['lastmagazine'] = $this->Mod_query->showDataMagazine($value);
			      	$data['submagazine']  = $this->Mod_query->subMagazine($value,$value1);
	   				$edisi  = $this->Mod_query->subMagazine($value,$value1)->issue_title;
		      		if(empty($value2)){
