...
class Ajaxrequest extends CI_Controller {
	...
	public function api_last_magazine(){
		$sintak = "select href from api_magazine LIMIT 1";
		$data  = $this->Mod_request->api_each_query($sintak);
		echo json_encode($data);
	}
	...
	{		
		...
		/**/
			...
			// $im->writeImage(str_replace('pdf', 'jpg', $pdfPath));
			#$im->clear();
			#$im->destroy();
			...
			*/
		    ...
		    for ($i = 0; $i < $num_pages; $i++) {
				...
				$im->setImageCompressionQuality(100);				
		            ...
		            $im->writeImage($saveAsPath . "temp/" . $a.'.jpg');
					#small image
		            //$im->resizeImage( 182, 240, Imagick::FILTER_LANCZOS, 0.1, false );    
