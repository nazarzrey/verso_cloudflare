...
class Ajaxrequest extends CI_Controller {
	...
	public function api_magz(){
		$sintak = "select lower(title) as title,path,description,href from api_magazine";
	...
	{		
		...
		/**/
		    ...
		    $foldername = "pageturner/".$imagename;
		    ...
		    for ($i = 0; $i < $num_pages; $i++) {
		            ...
		            #big image
					$im->resizeImage( 1240, 1654, Imagick::FILTER_LANCZOS, 0.1, false );    
		            $im->writeImage($saveAsPath . "page/" . $a.'.jpg');
					#small image
		            $im->resizeImage( 124, 165, Imagick::FILTER_LANCZOS, 0.1, false );    
		            $im->writeImage($saveAsPath . "thumb/" . $a.'.jpg');
		    }
		    $im->clear(); 
		    $im->destroy();    		
		    ...
		    $pdfname  	= $this->Mod_request->magz_update($value,$cover."/thumb/1.jpg");
		/**/
	}
}